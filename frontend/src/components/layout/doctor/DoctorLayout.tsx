    ...
    <div className={`pl-wrapper doctor-portal${theme === 'dark' ? ' dark' : ''}`}>
      ...
      {showPwdModal && (
        <div style={{
          ...
          <div className="pd-premium-modal" style={{ maxWidth: '440px', padding: '45px', textAlign: 'center' }}>
            {pwdSuccess ? (
              ...
              <>
                ...
                <form onSubmit={handlePwdSubmit} style={{ textAlign: 'left' }}>
                  ...
                  <button type="submit" disabled={pwdLoading} style={{ 
                    width: '100%', padding: '16px', borderRadius: '18px', background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)', color: 'white', border: 'none', fontWeight: 900, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', boxShadow: '0 15px 30px rgba(16, 185, 129, 0.25)', transition: 'all 0.3s'
                  }}
